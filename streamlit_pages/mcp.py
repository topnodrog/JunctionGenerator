...
def generate_mcp_config(ide_type, python_path, server_script_path):
    ...
    if ide_type == "Windsurf":
        ...
        return f"{python_path} {server_script_path}", f"docker run -i --rm --add-host=host.docker.internal:host-gateway -e GRAPH_SERVICE_URL=http://host.docker.internal:8100 JunctionGenerator-mcp:latest"
